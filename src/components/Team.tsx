      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 72, alignItems: "end", marginBottom: 64 }} className="team-intro">
        ...
        <Reveal delay="d2">
          <p style={{ fontSize: 17, fontWeight: 300, lineHeight: 1.8, color: "var(--text-mid)", maxWidth: 600 }}>
            We got into this business because financial security matters. And the people you choose to trust with yours matter even more.
          </p>
        </Reveal>
      </div>

      {/* Team — uniform 7-column grid */}
      <div style={{ display: "grid", gridTemplateColumns: "repeat(7,1fr)", gap: 16 }} className="team-grid">
        {teamMembers.map((m, i) => {
              ...
              >
                <div style={{ position: "relative", width: "100%", aspectRatio: "4/5", overflow: "hidden", background: "#0d2137" }}>
                  <Image
                    ...
                    alt={m.name}
                    fill
                    sizes="(max-width:640px) 50vw, (max-width:1100px) 25vw, 14vw"
